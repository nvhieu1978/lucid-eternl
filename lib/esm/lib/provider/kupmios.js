import { Addresses, fromUnit, Utils, } from "../mod.js";
export class Kupmios {
            ...
            client.addEventListener("message", (msg) => {
                try {
                    ...
                    Object.keys(result.plutusCostModels).forEach((v) => {
                        ...
                        costModels[plutusVersion] = Object.values(result.plutusCostModels[v]);
                    });
                    ...
                    res({
                        ...
                        maxTxExMem: parseInt(result.maxExecutionUnitsPerTransaction.memory),
                        maxTxExSteps: parseInt(result.maxExecutionUnitsPerTransaction.cpu),
    ...
    async getDelegation(rewardAddress) {
        const client = await this.rpc("queryLedgerState/delegateRepresentative", Addresses.inspect(rewardAddress).delegation?.type === "Key"
            ? { keys: [rewardAddress] }
            ...
            client.addEventListener("message", (msg) => {
                try {
                    ...
                    res({
                        ...
                        drep: "- information not available -", // TODO
