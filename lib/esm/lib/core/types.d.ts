import type { Credential, DatumVariant, DRep, Instruction, InstructionSigner, Network, RelevantProtocolParameters, Script, Utxo } from "../../rs_lib/pkg/lucid_core";
...
export type PartialInstruction = {
    ...
    redeemer?: string;
} | OmitInstruction<Instruction, "CollectFrom"> & OmitInstruction<Instruction, "ReadFrom"> & OmitInstruction<Instruction, "Withdraw">;
 ...
 */
export type OutputData = DatumVariant & {
    scriptRef?: Script;
} | {
    scriptRef: Script;
};
...
export interface Wallet {
    ...
    signMessage(address: string, payload: string): Promise<SignedMessage>;
    submit(tx: string): Promise<string>;
}
